...
def _require_admin():
    ...
    rid = getattr(request, "request_id", request_id())
    ...
    limiter = current_app.config.get("api_rate_limiter")
    if limiter and not limiter.allow("api:admin"):
        return jsonify(
            {
                "error": {"code": "rate_limited", "message": "Rate limit exceeded"},
                "request_id": rid,
...
def chat():
    ...
    if len(message) > config.MAX_API_MESSAGE_CHARS:
        return jsonify(
            {
                "error": {
                    "code": "invalid_request",
                    "message": f"message exceeds {config.MAX_API_MESSAGE_CHARS} characters",
                },
                "request_id": request.request_id,
            }
        ), 400

    # API identity is the user boundary. Never allow a caller to select an
    # arbitrary Telegram user ID and thereby access another user's memory.
    user_id = _stable_external_id(request.principal.key_id)
    service = current_app.config.get("chat_service")
    loop = current_app.config.get("bot_loop")
    if not service or not loop:
        return jsonify(
            {
                "error": {"code": "not_ready", "message": "chat service is not ready"},
                "request_id": request.request_id,
            }
        ), 503
    ...
    with metrics.timer("api.chat"):
        ...
        try:
            ...
            return jsonify(
                {
                    "error": {"code": "internal_error", "message": "AI request failed"},
                    "request_id": request.request_id,
                }
            ), 500
...
def create_key():
    ...
    if not isinstance(scopes, list) or not scopes or not set(scopes).issubset(allowed):
        return jsonify(
            {
                "error": {"code": "invalid_scopes", "message": "Unsupported scope"},
                "request_id": getattr(request, "request_id", request_id()),
            }
        ), 400

    key_id, token = current_app.config["api_key_store"].create(
        str(payload.get("name") or "client"), scopes
    )
    return jsonify(
        {
            "key_id": key_id,
...
def revoke_key(key_id: str):
    ...
    return jsonify(
        {
            "revoked": current_app.config["api_key_store"].revoke(key_id),
            "request_id": getattr(request, "request_id", request_id()),
        }
    )
